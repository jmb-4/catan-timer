...
test('4 players: 8 steps total (forward 4 + backward 4)', async () => {
  const { buildSetupSequence } = await import('../../party-timer/setup-logic.js');
...
test('usedColors returns empty array when upToIndex = 0', async () => {
  const { usedColors } = await import('../../party-timer/setup-logic.js');
...
test('nextAvailableColor skips already-used colors', async () => {
  const { nextAvailableColor } = await import('../../party-timer/setup-logic.js');
